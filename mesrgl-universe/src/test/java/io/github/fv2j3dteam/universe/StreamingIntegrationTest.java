...
class StreamingIntegrationTest {
    ...
    void manyRegionsLoadInOrder() throws Exception {
        ...
        try {
            var w = UniverseFactory.wire(UniverseId.universeRoot(1L), 42L,
                    io.github.fv2j3dteam.universe.seeds.SeedDerivation.GENERATOR_VERSION, p, 1_000_000L);
            try {
                for (int i = 0; i < 20; i++) {
                    final int idx = i;
    ...
    void repeatedTraversalRespectsMemory() throws Exception {
        ...
        try {
            var w = UniverseFactory.wire(UniverseId.universeRoot(1L), 42L,
                    io.github.fv2j3dteam.universe.seeds.SeedDerivation.GENERATOR_VERSION, p, 5_000L);
            try {
