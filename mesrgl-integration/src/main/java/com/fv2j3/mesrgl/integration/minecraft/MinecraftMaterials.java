...
public final class MinecraftMaterials {
    ...
    public MinecraftMaterials() {
        ...
        FaceUVs grassFaces = cube(
                tile(0, 0),  // +X east: grass side
                tile(0, 0),  // -X west
                tile(0, 0),  // +Y top: grass top
                tile(2, 0),  // -Y bottom: dirt
                tile(0, 0),  // +Z south
                tile(0, 0)   // -Z north
        );
        // grass-top has its own (col 0, row 0) of the 16x16 atlas; grass-side is
        // (col 0, row 3) in canonical terrain.png. We re-tile for correctness:
        grassFaces = new FaceUVs(
