...
public final class MesrGLRenderer implements AutoCloseable {
     ...
     *
     * The bridge keeps one scene per renderer (handle 1); destroy the old
     * one and immediately re-create so callers can keep using the constant
     * scene handle. If the re-create fails we surface the error rather
     * than silently leaving subsequent addMesh / addLight calls to
     * touch a dead scene.
     */
    public void resetScene() {
        MesrGLJNI.destroyScene(handle, 1);
        long recreated = MesrGLJNI.createScene(handle);
        if (recreated == 0) {
            String error = MesrGLJNI.getLastError(handle);
            throw new IllegalStateException("MesrGL scene re-create failed: " + error);
        }
        if (recreated != 1) {
            throw new IllegalStateException("MesrGL scene re-create returned unexpected handle: "
                    + recreated + " (renderer is not single-scene)");
        }
    }
